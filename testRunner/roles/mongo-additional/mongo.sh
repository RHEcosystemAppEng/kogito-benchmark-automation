...
if [ "$WHEN" = 'before' ]
then
  $CONTAINER_ENGINE cp roles/mongo-additional/mongoBefore.js kogito-mongodb:/tmp
...
if [ "$WHEN" = 'after' ]
then
  $CONTAINER_ENGINE cp roles/mongo-additional/mongoAfter.js kogito-mongodb:/tmp
#  podman exec -it kogito-mongodb "mongo -u developer -p mypass < /tmp/mongoBefore.js"
